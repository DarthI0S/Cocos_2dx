...
Scene* mini_game::createScene()
{
    auto scene = Scene::createWithPhysics();
    scene->getPhysicsWorld()->setDebugDrawMask(PhysicsWorld::DEBUGDRAW_ALL);

    auto layer = mini_game::create();
...
bool mini_game::init()
{
    ...
    listener->onKeyPressed = CC_CALLBACK_2(mini_game::onKeyPressed, this);
    listener->onKeyReleased = CC_CALLBACK_2(mini_game::onKeyReleased, this);
    _eventDispatcher->addEventListenerWithSceneGraphPriority(listener, this);

    this->schedule(CC_SCHEDULE_SELECTOR(mini_game::spawn_enemy), 1.0f); // Generar asteroides cada 1 segundo
    ...
    this->schedule([this](float delta) {
        ...
        if (remainingTime <= 0) {
            ...
            CC_CALLBACK_1(mini_game::menuPlayBack, this); // Llamar a la función de fin de juego
        }
...
void mini_game::spawn_enemy(float time) {
...
void mini_game::onKeyPressed(EventKeyboard::KeyCode keyCode, Event* event) {
    ...
    case EventKeyboard::KeyCode::KEY_RIGHT_ARROW:
        if (!movingRight) { // Evitar reiniciar la acción si ya se está moviendo
            ...
            moveRight->setTag(2); // Asignar un tag para identificar la acción
...
void mini_game::onKeyReleased(EventKeyboard::KeyCode keyCode, Event* event) {
...
void mini_game::bullet_asteroid() {
    auto contactListener = EventListenerPhysicsContact::create();
...
void mini_game::menuCloseCallback(Ref* pSender)
{

    Director::getInstance()->end();
}
void mini_game::menuPlayBack(Ref* pSender)
{
    AudioEngine::stopAll();


        Director::getInstance()->replaceScene(TransitionFade::create(0.2, Mapa_2::createScene()));
}

void mini_game::YouLose() {
    AudioEngine::stopAll();
