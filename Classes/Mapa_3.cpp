...
bool Mapa_3::init()
{
    ...
    vida = Sprite::create("salud.png");
    vida->setPosition(visibleSize.width / 2, 120); // Posición inicial
    vida->setTag(30); // Tag para identificar como item de salud
    ...
    contactListener->onContactBegin = [this](PhysicsContact& contact) -> bool {
        ...
        // Verificar si la colisión es entre el jugador y el ítem de salud
        if ((nodeA == player && nodeB->getTag() == 30) ||
            (nodeB == player && nodeA->getTag() == 30)) {

            CCLOG("Colisión: Jugador con ítem de salud");
            player->setHealth(player->getHealth() + 20); // Incrementar la salud del jugador

            // Eliminar el ítem de salud tras el contacto
            if (nodeA->getTag() == 30) nodeA->removeFromParentAndCleanup(true);
            if (nodeB->getTag() == 30) nodeB->removeFromParentAndCleanup(true);

            return true;
        }

        return false; // Continuar procesando otros contactos
        };
    _eventDispatcher->addEventListenerWithSceneGraphPriority(contactListener, this);


    auto vidaBody = PhysicsBody::createBox(vida->getContentSize(), PhysicsMaterial(0.0f, 0.0f, 0.0f));
    vidaBody->setDynamic(false); // El objeto no se mueve
    vidaBody->setContactTestBitmask(0xFFFFFFFF); // Detecta colisiones
    vida->setPhysicsBody(vidaBody);
    ...
    auto fondo = Sprite::create("fondo_3.png");
    ...
    //piso
    auto piso = Sprite::create("piso_4.png");
    ...
    //col_piso->setCategoryBitmask(0x01);  // Categoría del piso
    //col_piso->setCollisionBitmask(0x02); // Colisiona con enemigos
    //col_piso->setContactTestBitmask(0x02); // Detecta contacto con enemigos
    piso->setPhysicsBody(col_piso);
    this->schedule(CC_SCHEDULE_SELECTOR(Mapa_3::spawn_bullet_enemy), 0.2f); // Generar asteroides cada 1 segundo
    ...
    auto audio_level = AudioEngine::play2d("audio/level_3.mp3");
    ...
    listener->onKeyPressed = CC_CALLBACK_2(Mapa_3::onKeyPressed, this);
    listener->onKeyReleased = CC_CALLBACK_2(Mapa_3::onKeyReleased, this);
    _eventDispatcher->addEventListenerWithSceneGraphPriority(listener, this);
    cocos2d::EventListenerPhysicsContact* contactListener_ = cocos2d::EventListenerPhysicsContact::create();
    auto enemy_contactlis = EventListenerPhysicsContact::create();

    // Asignar el callback para detectar colisiones (tipo funcion)
    contactListener_->onContactBegin = [this, piso](PhysicsContact& contact) -> bool {
        auto bodyA = contact.getShapeA()->getBody();
        auto bodyB = contact.getShapeB()->getBody();
    ...
    _eventDispatcher->addEventListenerWithSceneGraphPriority(contactListener_, this);


    return true;
}

void Mapa_3::update(float delta) {
...
void Mapa_3::onKeyPressed(EventKeyboard::KeyCode keyCode, Event* event) {
...
void Mapa_3::onKeyReleased(EventKeyboard::KeyCode keyCode, Event* event) {
...
void Mapa_3::spawn_bullet_enemy(float delta) {
...
void Mapa_3::thanks() {
    AudioEngine::stopAll();
    Director::getInstance()->replaceScene(TransitionFade::create(0.1, thanks::create()));
